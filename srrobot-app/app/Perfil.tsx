...
const Perfil = () => {
  ...
  const insets = useSafeAreaInsets(); // Obtener insets para áreas seguras
  ...
  const swipeGestureRegistros = Gesture.Pan()
    .onEnd((event) => {
      if (event.translationX > 100) { // Solo deslizamiento a la derecha
        navigation.navigate('Registros');
      }
    })
    .minDistance(50); // Umbral mínimo para el gesto
  ...
  const swipeGestureHome = Gesture.Pan()
    .onEnd((event) => {
      if (event.translationX < -100) { // Solo deslizamiento a la izquierda
        refreshHome();
      }
    })
    .minDistance(50);
  ...
  const swipeGestureLogout = Gesture.Pan()
    .onEnd((event) => {
      if (event.translationY > 150) { // Deslizamiento hacia abajo más estricto
        handleLogout();
      }
    })
    .minDistance(50);
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1, backgroundColor: '#1F2937', position: 'relative' }}>
        ...
        <View style={{ flex: 1, paddingHorizontal: wp(6), paddingVertical: hp(2), paddingBottom: hp(8), alignItems: 'center', maxWidth: wp(100) }}>
          <Text style={{ color: '#FFFFFF', fontSize: wp(6), fontWeight: 'bold', marginBottom: hp(3) }}>Perfil</Text>
          {user ? (
            <View style={{ width: wp(90), backgroundColor: '#374151', padding: wp(4), borderRadius: wp(2), alignItems: 'center' }}>
              <Icon name="person" size={wp(12)} color="#60A5FA" style={{ marginBottom: hp(1.5) }} />
              <Text style={{ color: '#FFFFFF', fontSize: wp(4.2), fontWeight: '600', marginBottom: hp(0.8) }}>Nombre: {user.nombreCompleto}</Text>
              <Text style={{ color: '#9CA3AF', fontSize: wp(3.8), marginBottom: hp(0.8) }}>Usuario: {user.nombreUsuario}</Text>
          ...
          <GestureDetector gesture={swipeGestureLogout}>
            ...
            >
              <Text style={{ color: '#FFFFFF', fontSize: wp(4.2), fontWeight: 'bold', textAlign: 'center' }}>Cerrar Sesión</Text>
            </TouchableOpacity>
          </GestureDetector>
        </View>
        ...
        <View style={{
          ...
          paddingBottom: hp(1.5) + insets.bottom, // Ajuste dinámico para botones virtuales
          borderTopWidth: 1,
