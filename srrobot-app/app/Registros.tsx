...
const Registros = () => {
  ...
  const [stats, setStats] = useState<StatsData>({ totalEvents: 0, criticalEvents: 0, average: "0%" })
  ...
  const fetchData = async () => {
    try {
      ...
      const totalEvents = data.reduce((sum: number, item: EventData) => sum + item.eventosTotales, 0)
      const criticalEvents = data.reduce((sum: number, item: EventData) => sum + item.eventosCriticos, 0)
  ...
  const handleFilter = (period: string) => setFilter(period)
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1, backgroundColor: "#1F2937", position: "relative", paddingTop: insets.top }}>
        ...
        >
          ...
          >
            <Text style={{ color: "#FFFFFF", fontSize: wp(5.5), fontWeight: "bold" }}>
              Panel de Seguridad Sr. Robot
            </Text>
            <TouchableOpacity style={{ padding: wp(1.5) }} onPress={() => router.push("/Login")}>
              <Icon name="power-settings-new" size={wp(5.5)} color="#DC2626" />
            </TouchableOpacity>
          </View>

          {/* Historial de Eventos */}
          <Text style={{ color: "#FFFFFF", fontSize: wp(4.2), fontWeight: "bold", marginBottom: hp(0.8) }}>
            HISTORIAL DE EVENTOS
          </Text>
          <Text style={{ color: "#9CA3AF", fontSize: wp(3.2), marginBottom: hp(1.5) }}>
            Filtra los eventos por período de tiempo
          </Text>
          <View
            style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: hp(2.5), flexWrap: "wrap" }}
          >
          ...
          {/* Estadísticas */}
          <Text style={{ color: "#FFFFFF", fontSize: wp(4.2), fontWeight: "bold", marginBottom: hp(1.5) }}>
          ...
          <ScrollView style={{ flex: 1 }}>
            {events.map((event) => (
              ...
              >
                <View>
                  <Text style={{ color: "#9CA3AF", fontSize: wp(3.2) }}>
                    {new Date(event.marcaTiempo).toLocaleString()}
                  </Text>
                  <Text style={{ color: "#9CA3AF", fontSize: wp(3.2) }}>
                    Intensidad: {(event.promedio || 0).toFixed(2)}%
                  </Text>
                </View>
        ...
        >
          <GestureDetector gesture={swipeGestureHome}>
            <TouchableOpacity style={{ alignItems: "center", padding: wp(2) }} onPress={() => router.push("/Home")}>
              <Image
